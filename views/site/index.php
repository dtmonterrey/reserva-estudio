...
<div>
	...
	<div><i class="fa fa-check-square" style="color:green;"></i> <span style="text-decoration:line-through;">Painel de validação de pedidos</span></div>
...
<?php
	 ...
	 if (count($responsaveisEstudio) > 0) {
		...
		foreach ($reservas as $r) {
			...
			echo '<td>';
			echo \yii\helpers\Html::a($r->id, ['reserva/view', 'id'=>$r->id]);
			echo '</td>';
			echo "<td>$user->nome - $user->email</td><td>$estudio->nome_estudio</td><td>$r->inicio</td><td>$r->fim</td>";
			echo '<td>';
